...
class Amazon(BaseService):
    ...
    def __init__(self, ctx, asin, bitrate, cdn, quality, single, audio_manifest, audio_quality):
        self.title = asin
        if not self.title:
    ...
    def get_titles_vod(self, asin):
        ...
        for t in api_titles:
            if t["contentType"] == "MOVIE":
                titles.append(Title(
                    ...
                    name=title_name,
                    year=t.get("releaseOrFirstAiringDate", {}).get("valueFormatted", "")[:4],
                    source=self.ALIASES[0],
                    service_data=t,
                ))
        ...
        for t in titles:
            if t.season > 100:
                ...
                episode = t.episode
                if volume > 1:
                    episode += max(x.episode for x in titles if x.season == t.season - 1)
