    ...
    *MPL: FHD with Android L3, sporadically available with ChromeCDM
    ...
    def get_tracks(self, title):
        if self.vcodec == "H264":
            ...
            self.config["profiles"]["H264"]["MPL+HPL"] = (
                self.config["profiles"]["H264"]["MPL"] + self.config["profiles"]["H264"]["HPL"]
            )
                ...
                if self.cdm.device.security_level == 3 and self.cdm.device.type == LocalDevice.Types.ANDROID:
                    ...
                    if profile == "HPL" and max_quality >= 1080:
                        if "SEGMENT_MAP_2KEY" in manifest["video_tracks"][0]["streams"][0]["tags"]:
                            ...
                            manifest_720 = self.get_manifest(title, self.config["profiles"]["H264"]["HPL_720p"])
    ...
    def get_metadata(self, title_id):
        ...
        try:
            ...
            if "status" in metadata and metadata["status"] == "error":
                self.log.exit(
                    f" - Failed to fetch metadata for {title_id}, cookies might be expired. ({metadata['message']})"
                )
                raise
            return metadata
    ...
    def manifest_as_tracks(self, manifest, original_language=None):
        ...
        return Tracks(
            ...
            [AudioTrack(
                ...
                is_original_lang=is_close_match(
                    self.NF_LANG_MAP.get(x["language"], x["language"]),
                    [original_language]
                ),
                bitrate=x["bitrate"] * 1000,
                channels=x["channels"],
