...
from vinetrimmer.objects import AudioTrack, TextTrack, Title, Tracks
...
class AppleTVPlus(BaseService):
    ...
    def get_tracks(self, title):
        ...
        tracks = Tracks.from_m3u8(
            m3u8.load(stream_data["assets"]["hlsUrl"], headers=self.config["headers"]),
            lang=title.original_lang,
            source=self.ALIASES[0]
        )
        for track in tracks:
            track_data = track.extra
